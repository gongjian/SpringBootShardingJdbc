...
public final class ModuloTableShardingAlgorithm implements
	...
	public String doEqualSharding(final Collection<String> tableNames,
		...
		for (String each : tableNames) {
			if (each.endsWith((shardingValue.getValue() - 1) % 4 + "")) {
				return each;
			}
		}
	...
	public Collection<String> doInSharding(final Collection<String> tableNames,
		...
		for (Long value : shardingValue.getValues()) {
			for (String tableName : tableNames) {
				if (tableName.endsWith((value - 1) % 4 + "")) {
